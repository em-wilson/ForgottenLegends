...
REDIT(redit_olist)
{
	...
	for (vnum = pArea->min_vnum; vnum <= pArea->max_vnum; vnum++)
	{
		if ((pObjIndex = get_obj_index(vnum)))
		{
			...
			{
				found = TRUE;
				snprintf(buf, sizeof(buf), "[%5d] %-17.16s{x",
...
AEDIT(aedit_create)
{
	...
	{
		send_to_char("You do not have enough security to create an area.\n\r", ch);
		send_to_char("Please see Damian for vnum allocation.\n\r", ch);
		return FALSE;
	}
...
AEDIT(aedit_file)
{
	...
	for (test = area_first; test != NULL; test = test->next)
	{
		...
		{
			snprintf(buf, sizeof(buf), "%s: filename is already taken.", buf);
			send_to_char(buf, ch);
			return FALSE;
		}
	}
...
REDIT(redit_show)
{
	...
	if (!pRoom->extra_descr.empty())
	{
		...
		auto itl = pRoom->extra_descr;
		for (auto it = itl.begin(); it != itl.end(); it++)
		{
			auto ed = *it;
			if (it != itl.begin())
			{
			...
			strcat(buf1, ed->getKeyword().c_str());
		}
		strcat(buf1, "]\n\r");
	}
...
REDIT(redit_ed)
{
	...
	if (!str_cmp(command, "add"))
	{
		...
		ed->setKeyword(keyword);
		ed->setDescription("");
		pRoom->extra_descr.push_back(ed);
	...
	if (!str_cmp(command, "edit"))
	{
		...
		for (auto ed : pRoom->extra_descr)
		{
			if (is_name(keyword, ed->getKeyword().c_str()))
				break;
		}
	...
	if (!str_cmp(command, "delete"))
	{
		...
		if (ped) {
			auto pel = list<ExtraDescription *>(pRoom->extra_descr.begin(), pRoom->extra_descr.end());
			pel.remove(ped);
			pRoom->extra_descr = vector<ExtraDescription *>(pel.begin(), pel.end());
			delete ped;
		} else  {
			send_to_char("REdit:  Extra description keyword not found.\n\r", ch);
			return FALSE;
	...
	if (!str_cmp(command, "format"))
	{
		...
		ed->setDescription(format_string(ed->getDescription().c_str()));
...
REDIT(redit_oreset)
{
	...
	if (pObjIndex->item_type == ITEM_NUKE)
	{
		send_to_char("Nuclear weapons may not be added to area resets.\n\r", ch);
		return FALSE;
	}
...
void show_obj_values(Character *ch, OBJ_INDEX_DATA *obj)
{
	...
	case ITEM_WINDOW: /* Voltecs Window code! */
	...
	case ITEM_ARROW:
		snprintf(buf, sizeof(buf), "[v0] Arrows left:    %d\n\r", obj->value[0]);
		send_to_char(buf, ch);
		break;

	case ITEM_BOW:
		snprintf(buf, sizeof(buf), "[v0] Weapon range:   %d\n\r", obj->value[0]);
		send_to_char(buf, ch);
		snprintf(buf, sizeof(buf), "[v1] Number of dice: [%d]\n\r", obj->value[1]);
		send_to_char(buf, ch);
		snprintf(buf, sizeof(buf), "[v2] Type of dice:   [%d]\n\r", obj->value[2]);
...
bool set_obj_values(Character *ch, OBJ_INDEX_DATA *pObj, int value_num, char *argument)
{
	...
	case ITEM_ARMOR:
		...
		/* BANK NOTES by Blizzard */
	case ITEM_BANKNOTE:
		switch (value_num)
		{
		default:
			send_to_char("Either gold or silver.\n\r", ch);
			return FALSE;
		case 0:
...
OEDIT(oedit_show)
{
	...
	snprintf(buf, sizeof(buf), "Min Str:     [%d]\n\r", /* FL */
			 pObj->min_stat);
	send_to_char(buf, ch);
...
OEDIT(oedit_str)
{
	...
	if (atoi(argument) > 25)
	{
		send_to_char("25 is the highest strength can go.\n\r", ch);
		return FALSE;
	}

	pObj->min_stat = atoi(argument);

	send_to_char("Min strength set.\n\r", ch);
	return TRUE;
}
...
MEDIT(medit_show)
{
	...
	snprintf(buf, sizeof(buf), "Vnum:        [%5d] Sex:   [%s]   Race: [%s]\n\r",
			 pMob->vnum,
...
MEDIT(medit_race)
{
	...
	if (argument[0] != '\0' && ((race = race_manager->getRaceByName(argument))))
	{
		EDIT_MOB(ch, pMob);
