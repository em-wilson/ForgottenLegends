...
void spell_identify( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    case ITEM_BOW:
	sprintf(buf, "Weapon range is %d.\n\r", obj->value[0]);
	send_to_char(buf,ch);
	break;

    case ITEM_ARROW:
	sprintf(buf, "Arrows remaining: %d.\n\r", obj->value[0]);
	send_to_char(buf,ch);
	break;
...
/* Druid spells
   -Blizzard */
void spell_call_ent( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    CHAR_DATA *mob;
    int i;
    ...
    act( "$n calls an Ent to do his bidding!", ch, NULL, NULL, TO_ROOM );
    act( "An Ent has arrived to assist in your struggles.", ch, NULL, NULL, TO_CHAR );
...
void spell_oaken_strength( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    AFFECT_DATA af;
    ...
    if ( is_affected( victim, sn ) )
    {
        if (victim == ch)
          ...
          act("$N already has the strength of an oak tree.",ch,NULL,victim,TO_CHAR);
        return;
    }
    af.where     = TO_AFFECTS;
    af.type      = sn;
    ...
    if ( ch != victim )
        act("$N's muscles grow with the strength of an oak tree.",ch,NULL,victim,TO_CHAR);
    return;
}

void spell_sapling_health( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    int heal;
...
void spell_pine_health( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    int heal;
...
void spell_aspen_health( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    int heal;
...
void spell_redwood_health( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    int heal;
...
void spell_bark_skin( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
    ...
    if ( is_affected( victim, sn ) )
    {
        if (victim == ch)
          ...
          act("$N already has flakes of bark on their skin.",ch,NULL,victim,TO_CHAR);
        return;
    }
    af.where     = TO_AFFECTS;
    af.type      = sn;
