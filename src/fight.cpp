...
void check_assist(Character *ch, Character *victim)
{
	...
	for (rch = ch->in_room->people; rch != NULL; rch = rch_next)
	{
		...
		if (IS_AWAKE(rch) && rch->fighting == NULL && rch->can_see(victim))
		{
			...
			if (!ch->isNPC() && IS_NPC(rch) && IS_SET(rch->off_flags, ASSIST_PLAYERS) && rch->level + 6 > victim->level)
			{
			...
			/* PCs next */
			if (!ch->isNPC() || IS_AFFECTED(ch, AFF_CHARM))
			{
				if (((!IS_NPC(rch) && IS_SET(rch->act, PLR_AUTOASSIST)) || IS_AFFECTED(rch, AFF_CHARM)) && ch->isSameGroup(rch) && !is_safe(rch, victim))
					multi_hit(rch, victim, TYPE_UNDEFINED);
...

			{
				if ((IS_NPC(rch) && IS_SET(rch->off_flags, ASSIST_ALL))
					...
					|| (IS_NPC(rch) && rch->getRace() == ch->getRace() && IS_SET(rch->off_flags, ASSIST_RACE))

					|| (IS_NPC(rch) && IS_SET(rch->off_flags, ASSIST_ALIGN) && ((IS_GOOD(rch) && IS_GOOD(ch)) || (IS_EVIL(rch) && IS_EVIL(ch)) || (IS_NEUTRAL(rch) && IS_NEUTRAL(ch))))
...

				{
					...
					{
						if (rch->can_see(vch) && vch->isSameGroup(victim) && number_range(0, number) == 0)
						{
							target = vch;
							number++;
						}
...
void one_hit(Character *ch, Character *victim, int dt, bool secondary)
{
	 ...
	 * Calc damage.
	 */
	if (ch->isNPC() && (!ch->pIndexData->new_format || wield == NULL))
	{
		...
		if (ch->fighting == victim && IS_WEAPON_STAT(wield, WEAPON_POISON))
		{
			...
			if (poison != NULL)
			{
				...
				if (poison->level == 0 || poison->duration == 0)
					act("The poison on $p has worn off.", ch, wield, NULL, TO_CHAR, POS_RESTING);
			}
		}
			...
			bool show)
{
	...
	if (dam > 1 && !IS_NPC(victim) && ((PlayerCharacter*)victim)->condition[COND_DRUNK] > 10)
		dam = 9 * dam / 10;
	...
	if (victim->position == POS_DEAD)
	{
		...
		if (!IS_NPC(victim))
		{
			snprintf(log_buf, 2 * MAX_INPUT_LENGTH, "%s killed by %s at %d",
					 ...
					 (ch->isNPC() ? ch->short_descr : ch->getName().c_str()),
					 ch->in_room->vnum);
			 ...
			 */
			if (victim->exp > exp_per_level(victim, ((PlayerCharacter*)victim)->points) * victim->level)
				victim->gain_exp((2 * (exp_per_level(victim, ((PlayerCharacter*)victim)->points) * victim->level - victim->exp) / 3) + 50);
		}
 ...
 * Inflict damage from a hit.
 */
bool damage_old(Character *ch, Character *victim, int dam, int dt, int dam_type, bool show)
{
	bool immune;
	...
	if (victim != ch)
	{
		...
		if (victim->position > POS_STUNNED)
		{
			 ...
			 * If victim is charmed, ch might attack victim's master.
			 */
			if (ch->isNPC() && IS_NPC(victim) && IS_AFFECTED(victim, AFF_CHARM) && victim->master != NULL && victim->master->in_room == ch->in_room && number_bits(3) == 0)
			{
				stop_fighting(ch, FALSE);
				multi_hit(ch, victim->master, TYPE_UNDEFINED);
				return FALSE;
...
bool is_safe(Character *ch, Character *victim)
{
	...
	else
	{
		...
		else
		{
			...
			if (ch->isClanned() && !ch->getClan()->hasFlag(CLAN_PK))
			{
				send_to_char("You must be in a pkilling clan to kill other players.\n\r", ch);
				return TRUE;
			}

			if (victim->isClanned() && !victim->getClan()->hasFlag(CLAN_PK))
			{
...
void check_killer(Character *ch, Character *victim)
{
	 ...
	 */
	if (ch->isNPC() || ch == victim || ch->level >= LEVEL_IMMORTAL || !ch->isClanned() || ch->fighting == victim)
		return;
...
bool check_parry(Character *ch, Character *victim)
{
	...
	act("You {Dparry{x $n's attack.", ch, NULL, victim, TO_VICT, POS_RESTING);
	act("$N {Dparries{x your attack.", ch, NULL, victim, TO_CHAR, POS_RESTING);
...
bool check_shield_block(Character *ch, Character *victim)
{
	...
	act("You block $n's attack with your shield.", ch, NULL, victim, TO_VICT, POS_RESTING);
	act("$N blocks your attack with a shield.", ch, NULL, victim, TO_CHAR, POS_RESTING);
...
bool check_dodge(Character *ch, Character *victim)
{
	...
	act("You {Ddodge{x $n's attack.", ch, NULL, victim, TO_VICT, POS_RESTING);
	act("$N {Ddodges{x your attack.", ch, NULL, victim, TO_CHAR, POS_RESTING);
...
void death_cry(Character *ch)
{
	...
	{
	case 0:
		msg = (char *)"$n hits the ground ... DEAD.";
		break;
	case 1:
		if (ch->material == 0)
		{
...
void group_gain(Character *ch, Character *victim)
{
	...
	for (gch = ch->in_room->people; gch != NULL; gch = gch->next_in_room)
	{
		...
		for (auto obj : gch->getCarrying())
		{
			...
			if ((obj->hasStat(ITEM_ANTI_EVIL) && IS_EVIL(gch)) || (obj->hasStat(ITEM_ANTI_GOOD) && IS_GOOD(gch)) || (obj->hasStat(ITEM_ANTI_NEUTRAL) && IS_NEUTRAL(gch)))
			{
...
int xp_compute(Character *gch, Character *victim, int total_levels)
{
	...
	if (IS_CLANNED(gch))
	{
		if (gch->getClan()->hasFlag(CLAN_PK))
			xp *= 1.5;
		else
			xp *= 1.25;
	}
...
void dam_message(Character *ch, Character *victim, int dam, int dt, bool immune)
{
	...
	{
		vs = "{m***{r DEVASTATE{m ***{x";
		vp = "{m***{r DEVASTATES{m ***{x";
	}
	else if (dam <= 100)
	{
		vs = "{y==={r OBLITERATE{y ==={x";
		vp = "{y==={r OBLITERATES{y ==={x";
	}
	else if (dam <= 125)
	{
		vs = "{y>>>{r ANNIHILATE{y <<<{x";
		vp = "{y>>>{r ANNIHILATES{y <<<{x";
	}
	else if (dam <= 150)
	{
		vs = "{y<<<{r ERADICATE{y >>>{x";
		vp = "{y<<<{r ERADICATES{y >>>{x";
	}
	else
	{
		vs = "do {RUNSPEAKABLE{x things to";
...
void disarm(Character *ch, Character *victim)
{
	...
	act("$n {YDISARMS{x you and sends your weapon flying!", ch, NULL, victim, TO_VICT, POS_RESTING);
...
void do_berserk(Character *ch, char *argument)
{
	...
	if ((chance = get_skill(ch, gsn_berserk)) == 0 || (ch->isNPC() && !IS_SET(ch->off_flags, OFF_BERSERK)) || (!ch->isNPC() && ch->level < ((PlayerCharacter*)ch)->sk_level[gsn_berserk]))
	{
...
void do_bash(Character *ch, char *argument)
{
	...
	if ((chance = get_skill(ch, gsn_bash)) == 0 || (ch->isNPC() && !IS_SET(ch->off_flags, OFF_BASH)) || (!ch->isNPC() && ch->level < ((PlayerCharacter*)ch)->sk_level[gsn_bash]))
	{
	...
	if (IS_NPC(victim) &&
		...
		!ch->isSameGroup(victim->fighting))
	{
	...
	{ /*
	  act("$n tries to bash you, but you dodge it.",ch,NULL,victim,TO_VICT, POS_RESTING);
	  act("$N dodges your bash, you fall flat on your face.",ch,NULL,victim,TO_CHAR, POS_RESTING);
...
void do_dirt(Character *ch, char *argument)
{
	...
	if ((chance = get_skill(ch, gsn_dirt)) == 0 || (ch->isNPC() && !IS_SET(ch->off_flags, OFF_KICK_DIRT)) || (!ch->isNPC() && ch->level < ((PlayerCharacter*)ch)->sk_level[gsn_dirt]))
	{
...
void do_trip(Character *ch, char *argument)
{
	...
	if ((chance = get_skill(ch, gsn_trip)) == 0 || (ch->isNPC() && !IS_SET(ch->off_flags, OFF_TRIP)) || (!ch->isNPC() && ch->level < ((PlayerCharacter*)ch)->sk_level[gsn_trip]))
	{
...
void do_murder(Character *ch, char *argument)
{
	...
	if (IS_AFFECTED(ch, AFF_CHARM) || (ch->isNPC() && IS_SET(ch->act, ACT_PET)))
		return;
...
void do_backstab(Character *ch, char *argument)
{
	...
	if ((victim->hit < victim->max_hit / 3 && IS_AWAKE(victim)) || (victim->fighting == NULL && get_skill(ch, gsn_backstab) < number_range(1, 95)))
	{
	...
	if (number_range(1, 80) < get_skill(ch, gsn_backstab) || (get_skill(ch, gsn_backstab) >= 2 && !IS_AWAKE(victim)))
	{
...
void do_flee(Character *ch, char *argument)
{
	...
	for (attempt = 0; attempt < 6; attempt++)
	{
		...
		door = number_door();
		if ((pexit = was_in->exit[door]) == 0 || pexit->u1.to_room == NULL || IS_SET(pexit->exit_info, ExitFlag::ExitClosed) || number_range(0, ch->daze) != 0 || (ch->isNPC() && IS_SET(pexit->u1.to_room->room_flags, ROOM_NO_MOB)))
			continue;
		...
		if (!ch->isNPC())
		{
			...
			if ((ch->class_num == 2) && (number_percent() < 3 * (ch->level / 2)))
...
void do_rescue(Character *ch, char *argument)
{
	...
	if (!ch->isNPC() && IS_NPC(victim) && !ch->isSameGroup(victim))
	{
		act("$N doesn't need your help!", ch, NULL, victim, TO_CHAR, POS_RESTING);
		return;
	}
	...
	if (IS_NPC(fch) && !ch->isSameGroup(victim))
	{
		send_to_char("Kill stealing is not permitted.\n\r", ch);
		return;
	}
...
void do_kick(Character *ch, char *argument)
{
	...
	if (!ch->isNPC() && ch->level < ((PlayerCharacter*)ch)->sk_level[gsn_kick] && !IS_BUNNY(ch))
	{
		send_to_char(
	...
	if (ch->isNPC() && !IS_SET(ch->off_flags, OFF_KICK))
		return;
...
void do_disarm(Character *ch, char *argument)
{
	...
	if (ch->getEquipment(WEAR_WIELD) == NULL && ((hth = get_skill(ch, gsn_hand_to_hand)) == 0 || (ch->isNPC() && !IS_SET(ch->off_flags, OFF_DISARM))))
	{
...
void do_surrender(Character *ch, char *argument)
{
	...
	act("$n tries to surrender to $N!", ch, NULL, mob, TO_NOTVICT, POS_RESTING);
	stop_fighting(ch, TRUE);

	if (!ch->isNPC() && IS_NPC(mob) && (!HAS_TRIGGER(mob, TRIG_SURR) || !mp_percent_trigger(mob, ch, NULL, NULL, TRIG_SURR)))
	{
		act("$N seems to ignore your cowardly act!", ch, NULL, mob, TO_CHAR, POS_RESTING);
...
void do_bite(Character *ch, char *argument)
{
	...
	if (!ch->isNPC() && ch->level < ((PlayerCharacter*)ch)->sk_level[gsn_bite])
	{
		send_to_char(
...
void do_breath(Character *ch, char *argument)
{
	...
	if (is_safe(ch, victim))
	{
		act("You cannot attack $N.", ch, NULL, victim, TO_CHAR, POS_RESTING);
		return;
	}
...
void do_endure(Character *ch, char *arg)
{
	...
	if (ch->level < skill_table[gsn_endure].skill_level[ch->class_num] ||
		((PlayerCharacter*)ch)->learned[gsn_endure] <= 1)
	{
...
void do_tame(Character *ch, char *argument)
{
	...
	if (arg[0] == '\0')
	{
		...
		act("$n tries to tame $mself but fails miserably.", ch, NULL, NULL, TO_ROOM, POS_RESTING);
		return;
	}
	...
	if (!IS_NPC(victim))
	{
		act("$N is beyond taming.", ch, NULL, victim, TO_CHAR, POS_RESTING);
		return;
	}

	if (!IS_SET(victim->act, ACT_AGGRESSIVE))
	{
		act("$N is not usually aggressive.", ch, NULL, victim, TO_CHAR, POS_RESTING);
		return;
	}
	...
	if (number_percent() < get_skill(ch, gsn_tame) + 15 + 4 * (ch->level - victim->level))
	{
		...
		act("You calm $N down.", ch, NULL, victim, TO_CHAR, POS_RESTING);
		act("$n calms $N down.", ch, NULL, victim, TO_NOTVICT, POS_RESTING);
		stop_fighting(victim, TRUE);
	...
	{
		send_to_char("You failed.\n\r", ch);
		act("$n tries to calm down $N but fails.", ch, NULL, victim, TO_NOTVICT, POS_RESTING);
		act("$n tries to calm you down but fails.", ch, NULL, victim, TO_VICT, POS_RESTING);
...
void do_assassinate(Character *ch, char *argument)
{
	...
	if (ch->master != NULL && ch->isNPC())
		return;

	if (!ch->isNPC() && ch->level < skill_table[gsn_assassinate].skill_level[ch->class_num])
	{
	...
	{
		act("$N seems immune to your assassination attempt.", ch, NULL, victim, TO_CHAR, POS_RESTING);
		act("$N seems immune to $n's assassination attempt.", ch, NULL, victim, TO_ROOM, POS_RESTING);
		return;
	}

	WAIT_STATE(ch, skill_table[gsn_assassinate].beats);
	if (ch->isNPC() ||
		!IS_AWAKE(victim) || number_percent() < get_skill(ch, gsn_assassinate))
	...
	if (!(IS_NPC(victim)) && !(ch->isNPC()) && victim->position == POS_FIGHTING)
	{
		if (!victim->can_see(ch))
...
void do_strangle(Character *ch, char *argument)
{
	...
	{
		act("You grab hold of $N's neck and put $M to sleep.", ch, NULL, victim, TO_CHAR, POS_RESTING);
		act("$n grabs hold of your neck and puts you to sleep.", ch, NULL, victim, TO_VICT, POS_RESTING);
		act("$n grabs hold of $N's neck and puts $M to sleep.", ch, NULL, victim, TO_NOTVICT, POS_RESTING);
...
void do_blackjack(Character *ch, char *argument)
{
	...
	{
		act("$E is already asleep.", ch, NULL, victim, TO_CHAR, POS_RESTING);
		return;
	}
	...
	{
		act("You hit $N's head with a lead filled sack.", ch, NULL, victim, TO_CHAR, POS_RESTING);
		act("You feel a sudden pain erupts through your skull!", ch, NULL, victim, TO_VICT, POS_RESTING);
		act("$n whacks $N at the back of $S head with a heavy looking sack!  *OUCH*", ch, NULL, victim, TO_NOTVICT, POS_RESTING);
