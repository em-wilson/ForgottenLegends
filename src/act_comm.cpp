...
void do_replay(Character *ch, char *argument)
{
    ...
    PlayerCharacter *pch = (PlayerCharacter*)ch;
    if (buf_string(pch->buffer)[0] == '\0')
    {
        send_to_char("You have no tells to replay.\n\r", ch);
        return;
    }

    page_to_char(buf_string(pch->buffer), ch);
    clear_buf(pch->buffer);
}
...
void do_gossip(Character *ch, char *argument)
{
    ...
    else /* gossip message sent, turn gossip on if it isn't already */
    {
        ...
        snprintf(buf, sizeof(buf), "{cYou gossip '%s{c'{x\n\r", argument);
        send_to_char(buf, ch);
...
void do_grats(Character *ch, char *argument)
{
    ...
    else /* grats message sent, turn grats on if it isn't already */
    {
        ...
        snprintf(buf, sizeof(buf), "{yYou grats '%s{y'{x\n\r", argument);
        send_to_char(buf, ch);
        for (d = descriptor_list; d != NULL; d = d->next)
        {
            Character *victim;
...
void do_clantalk(Character *ch, char *argument)
{
    ...
    for (d = descriptor_list; d != NULL; d = d->next)
    {
        if (d->connected == ConnectedState::Playing &&
            d->character != ch &&
            clan_manager->isSameClan(ch, d->character) &&
...
void do_tell(Character *ch, char *argument)
{
    ...
    if (victim->desc == NULL && !IS_NPC(victim))
    {
        ...
        buf[0] = UPPER(buf[0]);
        add_buf(((PlayerCharacter*)victim)->buffer, buf);
        return;
    }
    ...
    if (!ch->isNPC() && IS_NPC(victim) && HAS_TRIGGER(victim, TRIG_SPEECH))
...
void do_yell(Character *ch, char *argument)
{
    ...
    for (d = descriptor_list; d != NULL; d = d->next)
    {
        if (d->connected == ConnectedState::Playing && d->character != ch && d->character->in_room != NULL && d->character->in_room->area == ch->in_room->area && !IS_SET(d->character->comm, COMM_QUIET))
        {
...
void do_quit(Character *caller, char *argument)
{
    ...
    if (ch->isClanned())
    {
        clan_manager->add_playtime(ch->getClan(), current_time - ch->logon);
    }
...
void do_split(Character *ch, char *argument)
{
    ...
    {
        if (gch->isSameGroup(ch) && !IS_AFFECTED(gch, AFF_CHARM))
            members++;
    }
    ...
    for (gch = ch->in_room->people; gch != NULL; gch = gch->next_in_room)
    {
        if (gch != ch && gch->isSameGroup(ch) && !IS_AFFECTED(gch, AFF_CHARM))
        {
...
void do_reclass(Character *caller, char *argument)
{
    ...
    {
        if (!IS_SET(ch->done, class_table[iClass].flag) && can_be_class(ch, iClass))
            count++;
    }
...
void do_info(Character *ch, char *argument)
{
    ...
    else /* info sent, turn info on if it isn't already */
    {
        ...
        /* Old format, re-integrated
              snprintf(buf, sizeof(buf), "{YYou INFO: {C'%s{C'{x\n\r", argument );
              send_to_char( buf, ch );
              snprintf(buf, sizeof(buf), "{Y$n INFO: {C'%s{C'{x", argument );
        */

        snprintf(buf, sizeof(buf), "{YINFO: {C'%s{C'{x\n\r", argument);
        send_to_char(buf, ch);
        snprintf(buf, sizeof(buf), "{YINFO: {C'%s{C'{x", argument);
