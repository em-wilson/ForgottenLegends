...
void list_group_chosen(Character *ch)
{
	...
	for (gn = 0; gn < MAX_GROUP; gn++)
	{
		...
		if (ch->gen_data->group_chosen[gn] && group_table[gn].rating[ch->class_num] > 0)
		{
			snprintf(buf, sizeof(buf), "%-18s %-5d ", group_table[gn].name,
					 group_table[gn].rating[ch->class_num]);
			send_to_char(buf, ch);
...
bool parse_gen_groups(Character *ch, char *argument)
{
	...
	if (!str_prefix(arg, "add"))
	{
		...
		if (sn != -1)
		{
			...
			if (skill_rating(ch, sn) < 1 || skill_table[sn].spell_fun != spell_null)
			{
...
int skill_level(Character *ch, int sn)
{
	...
	{
		/* Bunnies first */
		if (ch->morph_form == morph_lookup("bunny") || ch->morph_form == morph_lookup("werebunny"))
		{
		...
		/* Now wolves */
		if (ch->morph_form == morph_lookup("wolf") || ch->morph_form == morph_lookup("werewolf"))
		{
		...
		/* Now bears */
		if (ch->morph_form == morph_lookup("bear") || ch->morph_form == morph_lookup("werebear"))
		{
			/* They have genetic bite! :) */
			if (!str_cmp(skill_table[sn].name, "bite"))
			{
		...
		/* Now rats */
		if (ch->morph_form == morph_lookup("rat") || ch->morph_form == morph_lookup("skaven"))
		{
			/* They have genetic bite! :) */
			if (!str_cmp(skill_table[sn].name, "bite"))
			{
