...
void NonPlayerCharacter::update()
{
    if (this->position >= POS_STUNNED)
    {
        ...
        if (IS_NPC(this) && this->zone != NULL && this->zone != this->in_room->area && this->desc == NULL && this->fighting == NULL && !IS_AFFECTED(this, AFF_CHARM) && number_percent() < 5)
        {
...
void NonPlayerCharacter::perform_autoloot()
{
    if (IS_SET(this->act, ACT_PET) && this->master != NULL)
    {
        // The master should be doing this
        this->master->perform_autoloot();
    }
}

bool NonPlayerCharacter::isClanned() { return false; }

Clan * NonPlayerCharacter::getClan() { return nullptr; }

void NonPlayerCharacter::gain_condition( int iCond, int value ) { }