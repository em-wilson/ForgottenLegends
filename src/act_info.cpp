...
void do_help(Character *ch, char *argument)
{
    ...
    for (pHelp = help_first; pHelp != NULL; pHelp = pHelp->next)
    {
        ...
        if (is_name(argall, pHelp->keyword))
        {
            ...
            found = TRUE;
            /* small hack :) */
            if (ch->desc != NULL && ch->desc->connected != ConnectedState::Playing && ch->desc->connected != ConnectedState::GenGroups)
                break;
        }
    }
...
void do_who(Character *ch, char *argument)
{
    ...
    for (;;)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if (iClass == -1)
                {
                    Race *iRace = race_manager->getRaceByName(arg);
    ...
    for (d = descriptor_list; d != NULL; d = d->next)
    {
        ...
        race = ch->getRace()->getPlayerRace()->getWhoName();
        // if (wch->race == race_lookup("werefolk"))
        // {
        // if (IS_SET(wch->act, PLR_IS_MORPHED))
        //     race = str_dup(morph_table[wch->morph_form].who_name);
...
void do_compare(Character *ch, char *argument)
{
    ...
    {
        if (value1 == value2)
            strncpy(msg, "$p and $P look about the same.", MAX_STRING_LENGTH);
        else if (value1 > value2)
            strncpy(msg, "$p looks better than $P.", MAX_STRING_LENGTH);
        else
...
void do_consider(Character *ch, char *argument)
{
    ...
    if (diff <= -10)
        strncpy(msg, "You can kill $N naked and weaponless.", MAX_STRING_LENGTH);
    else if (diff <= -5)
        strncpy(msg, "$N is no match for you.", MAX_STRING_LENGTH);
    else if (diff <= -2)
        strncpy(msg, "$N looks like an easy kill.", MAX_STRING_LENGTH);
    else if (diff <= 1)
        strncpy(msg, "The perfect match!", MAX_STRING_LENGTH);
    else if (diff <= 4)
        strncpy(msg, "$N says 'Do you feel lucky, punk?'.", MAX_STRING_LENGTH);
    else if (diff <= 9)
        strncpy(msg, "$N laughs at you mercilessly.", MAX_STRING_LENGTH);
    else
